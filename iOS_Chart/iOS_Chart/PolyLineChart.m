...
@interface PolyLineChart()<PolyLineDataSource,CAAnimationDelegate,UIGestureRecognizerDelegate>
...
@property(nonatomic, assign)CGFloat polyLineWidth;//折线线条粗细设置
...
@synthesize yAxisLineWidth,xAxisLineWidth,polyLineWidth;
...
@synthesize xAxisStrFontSize,yAxisStrFontSize;
...
- (void)drawYAxis
{
    ...
    [xAxisBezier moveToPoint:CGPointMake(yAxisWidth, self.height - chartBottomPadding + yAxisLayer.lineWidth/2.0)];
    [xAxisBezier addLineToPoint:CGPointMake(yAxisWidth, chartBottomPadding)];
    if (hasArrow) {
        [xAxisBezier moveToPoint:CGPointMake(yAxisWidth - arrowYOffset, yAxisWidth + arrowXOffset)];
        [xAxisBezier addLineToPoint:CGPointMake(yAxisWidth, chartTopPadding)];
        [xAxisBezier addLineToPoint:CGPointMake(yAxisWidth + arrowYOffset, chartTopPadding + arrowXOffset)];
    }
    ...
    yAxisLayer.path = xAxisBezier.CGPath;
...
- (void)reloadData
{
    @autoreleasepool {
        if (_dataSource) {
            ...
            if ([_dataSource respondsToSelector:@selector(axisColorOfPolyLineChart:)]) {
                axisColor = [_dataSource axisColorOfPolyLineChart:self];
                xAxisLayer.strokeColor = axisColor.CGColor;
                yAxisLayer.strokeColor = axisColor.CGColor;
            }
            
            NSUInteger pointCount = [_dataSource numbersOfPolylineChart:self];
            
            for (NSUInteger index = 0; index<pointCount; index++) {
                CGFloat value = [_dataSource polyLineChart:self valueOfIndex:index];
            ...
            for (NSUInteger index = 0; index<pointCount; index++) {
                ...
                NSString *text = @"";
                if ([_dataSource respondsToSelector:@selector(polyLineChart:titleOfValuesOfIndex:)]) {
                    text = [_dataSource polyLineChart:self titleOfValuesOfIndex:index];
...
- (void)drawYAxisGridAndUnitWithStartPoint:(CGPoint)startPoint
    ...
    for (NSUInteger index = 0; index<= sectionCount; index++) {
        ...
        textLayer.font = CGFontCreateWithFontName((__bridge CFStringRef)[[UIFont systemFontOfSize:13.f]fontName]);
        textLayer.alignmentMode = kCAAlignmentCenter;
        textLayer.backgroundColor = [UIColor clearColor].CGColor;
        textLayer.frame = CGRectMake(0, 0, yAxisWidth, 20);
        textLayer.position = CGPointMake(yAxisWidth/2.0, self.height - chartTopPadding - sectionHeight*(index));
    ...
    NSString *unitText = @"";
    if ([_dataSource respondsToSelector:@selector(YAxisUnitOfPolyLineChart:)]) {
        unitText = [_dataSource YAxisUnitOfPolyLineChart:self];
    }
...
- (void)settingChartLayerAndAnimate {
    ...
    if ([_dataSource respondsToSelector:@selector(fillColorOfPolyLineChart:)]) {
       fillColor = [_dataSource fillColorOfPolyLineChart:self];
    }

    PolyLineLayer *polyLineLayer_fill = [PolyLineLayer layer];
    polyLineLayer_fill.type = PolyLineFill;
    polyLineLayer_fill.lineCap = kCALineCapRound;
    polyLineLayer_fill.lineWidth = polyLineWidth;
    polyLineLayer_fill.fillColor = fillColor.CGColor;
    polyLineLayer_fill.strokeColor = [UIColor clearColor].CGColor;
    polyLineLayer_fill.lineJoin = kCALineJoinRound;
    polyLineLayer_fill.frame = CGRectMake(0, 0, xAxisLayer.frame.size.width, self.height - chartBottomPadding);
    polyLineLayer_fill.dataSource = self;
    if (hasFill) {
        [xAxisLayer addSublayer:polyLineLayer_fill];
    }
    [polyLineLayer_fill setNeedsDisplay];
    ...
    if (hasGradiant) {
       gradient.mask = polyLineLayer_fill;
    }
    ...
    CGPoint animateStartPoint = CGPointMake(firstPoint.x + polyLineLayer_fill.lineWidth/2.0, self.height - chartBottomPadding);
    ...
    UIGraphicsBeginImageContext(polyLineLayer_fill.frame.size);
    [beizer_animate fill];
    UIGraphicsEndImageContext();

    UIColor *lineColor = [UIColor orangeColor];
    if ([_dataSource respondsToSelector:@selector(lineColorOfPolyLineChart:)]) {
       lineColor = [_dataSource lineColorOfPolyLineChart:self];
    }

    PolyLineLayer *polyLineLayer_stroke = [PolyLineLayer layer];
    polyLineLayer_stroke.type = PolyLineStroke;
    polyLineLayer_stroke.lineCap = kCALineCapRound;
    polyLineLayer_stroke.lineWidth = polyLineWidth;
    polyLineLayer_stroke.fillColor = [UIColor clearColor].CGColor;
    polyLineLayer_stroke.strokeColor = lineColor.CGColor;
    polyLineLayer_stroke.lineJoin = kCALineJoinRound;
    polyLineLayer_stroke.frame = CGRectMake(0, 0, xAxisLayer.frame.size.width - chartRightPadding - chartRightPadding/2.0, self.height - chartBottomPadding);
    polyLineLayer_stroke.dataSource = self;
    if ([_dataSource respondsToSelector:@selector(showWithAnimateOfPolyLineChart:)]) {
       if([_dataSource showWithAnimateOfPolyLineChart:self]) {
           polyLineLayer_stroke.strokeEnd = 0.f;
       } else {
           polyLineLayer_stroke.strokeEnd = 1.f;
       }
    }
    if (hasStroke) {
       [xAxisLayer addSublayer:polyLineLayer_stroke];
    }
    [polyLineLayer_stroke setNeedsDisplay];

    PolyLineLayer *polyLineLayer_point = [PolyLineLayer layer];
    polyLineLayer_point.type = PolyLinePoint;
    polyLineLayer_point.lineCap = kCALineCapRound;
    polyLineLayer_point.lineWidth = polyLineWidth;
    polyLineLayer_point.fillColor = lineColor.CGColor;
    polyLineLayer_point.strokeColor = [UIColor clearColor].CGColor;
    polyLineLayer_point.lineJoin = kCALineJoinRound;
    polyLineLayer_point.frame = CGRectMake(0, 0, xAxisLayer.frame.size.width - chartRightPadding - chartRightPadding/2.0, self.height - chartBottomPadding);
    if ([_dataSource respondsToSelector:@selector(showWithAnimateOfPolyLineChart:)]) {
       if([_dataSource showWithAnimateOfPolyLineChart:self]) {
           polyLineLayer_point.opacity = 0.f;
       } else {
           polyLineLayer_point.opacity = 1.f;
       }
    }
    polyLineLayer_point.dataSource = self;
    if (hasPoint) {
        [xAxisLayer addSublayer:polyLineLayer_point];
    }
    [polyLineLayer_point setNeedsDisplay];
    ...
    [animate_path setValue:polyLineLayer_stroke forKey:@"animateLayer"];

    BOOL isAnimate = YES;
    if ([_dataSource respondsToSelector:@selector(showWithAnimateOfPolyLineChart:)]) {
       isAnimate = [_dataSource showWithAnimateOfPolyLineChart:self];
    }
    if (isAnimate) [polyLineLayer_fill addAnimation:animate_path forKey:@"path"];
}
...
- (void)settingChartTitle {
    ...
    xUnitTextLayer.font = fontRef;
    CFRelease(strRef);
    CGFontRelease(fontRef);
    xUnitTextLayer.alignmentMode = kCAAlignmentCenter;
    xUnitTextLayer.backgroundColor = [UIColor clearColor].CGColor;
    NSString *xUnitText = @"";
    if ([_dataSource respondsToSelector:@selector(xAxisUnitOfPolyLineChart:)]) {
        xUnitText = [_dataSource xAxisUnitOfPolyLineChart:self];
    }
...
#pragma mark -PolyLineDataSource
...
- (void)animationDidStop:(CAAnimation *)anim finished:(BOOL)flag
{
    if ([anim valueForKey:@"animateLayer"]) {
        PolyLineLayer *shape = (PolyLineLayer *)[anim valueForKey:@"animateLayer"];
...
- (void)refreshPolyLinePointsPosition
{
    @autoreleasepool {
        ...
        for (id layer in xAxisLayer.sublayers) {
            if ([layer isKindOfClass:[PolyLineLayer class]]) {
...
- (void)drawXAxisTitleWithPoints:(NSArray *)pointArray titles:(NSArray *)titleArray
{
    ...
    if ([_dataSource respondsToSelector:@selector(xAxisTextColorOfPolyLineChart:)]) {
        xTitleColor = [_dataSource xAxisTextColorOfPolyLineChart:self];
    }
    //计算合适的字号
...
- (void)panGesAction:(UIPanGestureRecognizer *)pan
{
    ...
    if (!CGRectContainsPoint(CGRectMake(chartLeftPadding, chartTopPadding, self.width - chartLeftPadding, self.height - chartTopPadding - chartBottomPadding), p)) {
        isMove = NO;
        [self.crossLineLayer setNeedsDisplay];
        return;
    }
...
- (void)doubleTapAction:(UITapGestureRecognizer *)tap {
    if (!([_dataSource respondsToSelector:@selector(supportFullScreenShowOfPolyLineChart:)] && [_dataSource supportFullScreenShowOfPolyLineChart:self])) {
        return;
    }
    PolyFullShowController *fullScreenVC = [PolyFullShowController new];
...
- (CAShapeLayer *)crossLineLayer
{
    if (!_crossLineLayer) {
        ...
        NSString *text = [NSString stringWithFormat:@"%@: %@\n%@: %.2f",[_dataSource xAxisUnitOfPolyLineChart:self],[_dataSource polyLineChart:self titleOfValuesOfIndex:index],[_dataSource YAxisUnitOfPolyLineChart:self],[_dataSource polyLineChart:self valueOfIndex:index]];
